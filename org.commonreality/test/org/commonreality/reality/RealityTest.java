...
public class RealityTest extends TestCase
{
  ...
  protected void setupConnection(IReality reality) throws Exception
  {
     ...
     */
    IMINATransportProvider local = new LocalTransportProvider();
    IMINAProtocolConfiguration protocol = new NOOPProtocol();
    ((DefaultReality) reality).addServerService(local, protocol, local
        .createAddress(1));
  }
...
//    _reality.shutdown();
  ...
  protected ISensor createSensor() throws Exception
  {
    ...
    IMINATransportProvider local = new LocalTransportProvider();
    IMINAProtocolConfiguration protocol = new NOOPProtocol();
    participant.addClientService(local, protocol, address);
